...
const texts = {
    ...
    como: {
      ...
      sections: [
        ...
        },
        {
          t: 'Especialización',
          d:
            'Aquí no hay masajes que sólo esconden el problema. Trabajo con ejercicio progresivo y fuerza real: el único camino que revierte lesiones y construye cuerpos resilientes. Cada sesión se adapta a tu estado actual. Si hoy tienes molestias, ajustamos. Si progresas, subimos el nivel.',
        },
      ],
      quote:
        '“Utilizo tecnología digital de precisión que mide cada movimiento, almacena tus resultados y compara la evolución semana a semana. Siempre sabes dónde estás. Siempre sabes si mejoras. Esto es readaptación de verdad.”',
    },
    quien: {
      title: 'Quién soy',
      content: [
  ...
  cat: {
    ...
    como: {
      ...
      quote:
        '“Utilitzo tecnologia digital de precisió que mesura cada moviment, emmagatzema els teus resultats i compara l’evolució setmana a setmana. Sempre saps on ets. Sempre saps si millores. Això és readaptació de veritat.”',
    },
    quien: {
      title: 'Qui sóc',
      content: [
        ...
        'Readaptació real. Resultats reals. Acompanyament humà.',
      ],
    },
    testimonios: {
      title: 'Testimonis',
...
function IndexPage() {
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900">
      ...
      {/* Servicios - planes online con precios y características */}
            ...
            {t.servicios.plans.map((p) => (
              <Card key={p.t} className="bg-white border border-zinc-200 rounded-xl shadow-sm">
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center gap-3">
                    <div className="text-2xl font-bold text-zinc-900">{p.priceMonthly}</div>
                    <div className="text-sm text-zinc-600">{p.priceQuarterly}</div>
                  </div>
                  <p className="text-zinc-700">{p.quote}</p>
      ...
      {/* Metodología - secciones según PDF + cita */}
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-2">
            {t.como.sections.map((s) => (
              <Card key={s.t} className="bg-white border border-zinc-200 rounded-xl shadow-sm">
                ...
                <CardContent className="text-zinc-700">{s.d}</CardContent>
              </Card>
            ))}
          </div>

          <div className="mt-8 rounded-xl bg-white border border-zinc-200 p-6">
            <p className="text-zinc-700 italic">{t.como.quote}</p>
          </div>
        </div>
      </section>
