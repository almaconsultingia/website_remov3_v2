...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900">
      ...
      {/* Metodología con flechas de secuencia */}
      <section id="como" className="container-max py-14 md:py-20">
        <h2 className="text-3xl md:text-4xl font-bold">{t.como.title}</h2>
        <div className="mt-8 flex flex-col md:flex-row md:flex-nowrap items-center md:items-stretch gap-4">
          {t.como.steps.map((s, idx) => (
            <div key={s.t} className="flex items-center w-full md:w-auto gap-4">
              <Card className="border border-zinc-200 flex-1">
                <CardHeader>
                  <CardTitle className="text-xl">{s.t}</CardTitle>
                </CardHeader>
                <CardContent className="text-zinc-600">{s.d}</CardContent>
              </Card>
              {idx < t.como.steps.length - 1 && (
                <div className="flex flex-col items-center justify-center">
                  <span className="block md:hidden text-2xl text-zinc-400" aria-hidden="true">↓</span>
                  <span className="hidden md:block text-3xl text-zinc-400" aria-hidden="true">→</span>
                </div>
              )}
            </div>
          ))}
            ...
            <CardContent className="p-0">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <img
                  ...
                  alt="Referent visual REMOV:3"
                  className="w-full h-64 md:h-full object-cover"
                  loading="lazy"
                />
                <img
                  src="/assets/quien-soy-physio.jpg"
                  alt="Fisioterapeuta jove"
                  className="w-full h-64 md:h-full object-cover"
                  loading="lazy"
                />
              </div>
        ...
        <div className="container-max py-10 grid gap-8 md:grid-cols-3">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-zinc-300">
              {lang === 'es'
                ? texts.es.footer.left_tagline
                : texts.cat.footer.left_tagline}
            </p>
          </div>
